...
const copyDir = (source, dest, { skip, veryVerbose } = {}) => {
  return new Promise((resolve, reject) => {
    const copied = []
    const skipped = []
...
const createProject = async function({ contract, frontend, projectDir, veryVerbose }) {
  ...
  await renameFile(`${projectDir}/dot.gitignore`, `${projectDir}/.gitignore`)
...
const opts = yargs
  .strict()
  .usage('$0 <projectDir>', 'Create a new DOT project')
